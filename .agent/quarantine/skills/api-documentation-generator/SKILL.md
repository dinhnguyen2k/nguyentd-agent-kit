...
description: 'Use when generating or refreshing developer-facing API documentation from existing endpoints, schemas, or OpenAPI artifacts.'
metadata:
...
For Cogain, default to ASP.NET Core REST/OpenAPI and document gRPC separately when proto contracts are in scope. GraphQL examples are only relevant when an approved GraphQL contract exists.
