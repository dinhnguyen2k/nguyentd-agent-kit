...
- Provide representative queries (1-3) before cost/performance recommendations.
- Separate OLTP and report workloads; do not overfit all optimizations to OLTP.
- Include at least one before/after plan evidence (`EXPLAIN ANALYZE` or equivalent) for a primary query.
- Document trade-off: cost saved vs latency/throughput risk.
- Document rollback/remediation path for risky index/schema changes.
