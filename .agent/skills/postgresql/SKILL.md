---
name: postgresql
description: Designs and reviews PostgreSQL schemas, data types, indexes, constraints, and query patterns. Use for PostgreSQL-specific data modeling or persistence decisions; not generic database administration.
metadata:
...
## PostgreSQL Hard DoD Extension (Mandatory)

When PostgreSQL schema/query/index changes are involved, all conditions below must pass:

1. Access patterns are listed

- Affected filter/join/sort/paging paths must be explicit.
- Include 1-3 representative queries (`list`, `detail`, `export/report`, `background job` where relevant).

2. Index-to-query mapping exists

- Each new/changed index must map to concrete query predicates/order.
- Composite index order must match query shape (equality first, then range/sort as needed).

3. FK indexing rule is enforced

- New FK columns are indexed by default.
- Exception requires documented rationale.

4. Plan evidence exists for hot/representative queries

- Provide before/after `EXPLAIN ANALYZE` (or equivalent), timing, rows scanned, index usage.
- Avoid avoidable `Seq Scan` / costly `Nested Loop` on large tables.

5. Migration risk is documented

- Include rollback/remediation plan.
- Include lock-risk notes and safe deployment approach.

6. Large index build uses safe method

- Use `CREATE INDEX CONCURRENTLY` when appropriate.
- Remember: `CREATE INDEX CONCURRENTLY` cannot run inside a transaction.

## Workload-Aware Guidance

- OLTP path: selective filters + small paging + low latency.
- Report path: wide scans/aggregations; may need pre-aggregation/materialization/query rewrite in addition to indexing.
- Do not evaluate all queries using only OLTP assumptions.
